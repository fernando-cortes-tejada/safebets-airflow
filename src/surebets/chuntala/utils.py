...
from ..entities import DICT_NBA_TEAMS, DICT_BETANO_CHUNTALA, DF_COLS


def get_game_list(driver: webdriver.Chrome) -> WebElement | None:
    try:
        game_list = driver.find_element(
            by=By.CLASS_NAME, value="sportsbook-game-list-wrapper__games"
        )
    except:
        print("Chuntala: game list not found")
        driver.close()
        return None
    else:
        print("game list ok")
...
def get_days(driver: webdriver.Chrome) -> list | None:
    try:
        days = driver.find_elements(by=By.CLASS_NAME, value="gamesWrapper")
    except:
        print("Chuntala: days not found")
        driver.close()
        return None
    else:
...
def get_games(days: list) -> list | None:
    try:
        for day in days:
            if "games" in locals():
                games += day.find_elements(by=By.XPATH, value="*")
            else:
                games = day.find_elements(by=By.XPATH, value="*")
    except:
        print("Chuntala: games not found")
        return None
    else:
